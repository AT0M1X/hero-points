...
- The manager keeps a Foundry-native surface treatment with gold accents for persistent/primary actions and blue accents for session/active-roster states; preserve that visual language when extending its UI. Successful awards and session starts pulse changed pool/total pills and briefly confirm on their action buttons. Award attempts that cannot change the selected pool use inline limit feedback rather than a Foundry notification. Session start uses a styled preview dialog showing eligible and persistent-maximum counts before applying the reset.
