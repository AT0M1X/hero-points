...
- The manager keeps a Foundry-native surface treatment with gold accents for persistent/primary actions and blue accents for session/active-roster states; preserve that visual language when extending its UI. Successful awards pulse the changed pool and total pills and briefly confirm on the action button.
